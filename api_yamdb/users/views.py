...
class APIToken(APIView):
    ...
    def post(self, request):
        serializer = UserSerializer(data=request.data)
        user = get_object_or_404(User, username=serializer.initial_data['username'])
            # проверяем confirmation code, если верный, выдаем токен
        if default_token_generator.check_token(user, serializer.initial_data['confirmation_code']):
